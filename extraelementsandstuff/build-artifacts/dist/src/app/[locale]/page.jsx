          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {featuredCategories.map((category) => {
                var _a;
                return (<div key={category.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
                  <div className="text-4xl mb-4">{category.icon}</div>
                  <h3 className="text-xl font-semibold mb-2">{category.title}</h3>
                  <p className="text-gray-600 mb-4">{category.description}</p>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">
                      {(_a = category._count) === null || _a === void 0 ? void 0 : _a.reports} reports
                    </span>
                    <a href={`/${locale}/categories/${category.slug}`} className="text-blue-600 hover:text-blue-800 font-medium">
                      View Reports →
                    </a>
                  </div>
                </div>);
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {featuredReportsData.reports.slice(0, 8).map((report) => {
                var _a;
                return (<div key={report.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                  <div className="p-6">
                    <div className="text-sm text-blue-600 font-medium mb-2">
                      {(_a = report.category) === null || _a === void 0 ? void 0 : _a.title}
                    </div>
                    <h3 className="text-lg font-semibold text-gray-900 mb-3 line-clamp-2">
                      <a href={`/${locale}/reports/${report.slug}`} className="hover:text-blue-600 transition-colors">
                        {report.title}
                      </a>
                    </h3>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <a href={`/${locale}/reports/${report.slug}`} className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 transition-colors">
                        View Report
                      </a>
                    </div>
                  </div>
