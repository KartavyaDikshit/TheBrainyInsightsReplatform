...
export async function GET() {
    ...
    try {
        ...
        const metrics = {
            ...
            hit_rate: calculateHitRate(extractValue(stats, 'keyspace_hits'), extractValue(stats, 'keyspace_misses'))
        };
        // Check cache keys
        const cacheKeysCount = {
            nextjs_cache: await client.eval("return #redis.call('keys', 'next-shared-cache*')", 0),
            api_cache: await client.eval("return #redis.call('keys', 'api:*')", 0),
            prisma_cache: await client.eval("return #redis.call('keys', 'prisma:*')", 0),
            session_cache: await client.eval("return #redis.call('keys', 'user:*')", 0),
        };
        const health = {
            status: 'healthy',
            timestamp: new Date().toISOString(),
...
function extractValue(info, key) {
